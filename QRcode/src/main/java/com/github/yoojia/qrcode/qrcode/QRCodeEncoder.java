...
public class QRCodeEncoder {
    ...
    public Bitmap encode(String content) {
        ...
        hints.put(EncodeHintType.CHARACTER_SET, mCharset);                 // 指定编码格式
        hints.put(EncodeHintType.MARGIN, mMargin);                         // 设置白边
        hints.put(EncodeHintType.ERROR_CORRECTION, mErrorCorrectionLevel); // 指定纠错等级
        BitMatrix result;
        try {
    ...
    public static class Builder {
        ...
        private int backgroundColor = DEF_BACKGROUND_COLOR;                         // 背景色
        private int pointColor = DEF_POINT_COLOR;                                   // 前景色
        private int padding = 0;                                                    // logo内边距
        private int margin = 0;                                                     // 二维码白边大小，取值范围0~4
        private int width = 500;                                                    // 二维码宽度
        private int height = 500;                                                   // 二维码高度
        private ErrorCorrectionLevel errorCorrectionLevel = ErrorCorrectionLevel.H; // 二维码容错率
